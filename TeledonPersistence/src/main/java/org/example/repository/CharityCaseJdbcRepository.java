...
public class CharityCaseJdbcRepository implements CharityCaseRepository {
    ...
    public CharityCase findOne(Long id) {
        ...
        try (PreparedStatement preStmt = con.prepareStatement("SELECT * FROM Cazuri WHERE id = ?")) {
            preStmt.setLong(1, id);
            try (ResultSet result = preStmt.executeQuery()) {
                if (result.next()) {
                    String name = result.getString("nume");
                    double amount = result.getDouble("suma");
                    CharityCase c = new CharityCase(name, amount);
                    c.setId(id);
                    return c;
                }
            }
