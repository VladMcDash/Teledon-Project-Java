...
public class DonorJdbcRepository implements DonorRepository {
    ...
    @Override
    public void update(Long id, String numeNou, String adresaNoua, String telefonNou) {
        Connection con = dbUtils.getConnection();
        try (PreparedStatement preStmt = con.prepareStatement("UPDATE Donors SET name=?, address=?, phoneNumber=? WHERE id=?")) {
            preStmt.setString(1, numeNou);
            preStmt.setString(2, adresaNoua);
            preStmt.setString(3, telefonNou);
            preStmt.setLong(4, id);
            preStmt.executeUpdate();
        } catch (SQLException e) {
            System.err.println("Eroare update donator DB: " + e);
        }
    }
